...
from django.shortcuts import redirect  # ← AGREGAR
...
def google_oauth_callback(request):
    ...
    try:
        ...
        # 6. Si es GET (navegador), redirigir a la página de éxito con los datos
        if request.method == 'GET':
        ...
        # 7. Si es POST (API), devolver JSON
        return Response({
            'message': 'Login exitoso con Google' if not created else 'Cuenta creada exitosamente',
            'access': access_token,
            'refresh': str(refresh),
            'user': {
                'id': user.id,
        ...
        return Response({'error': 'Timeout al comunicarse con Google'}, status=status.HTTP_504_GATEWAY_TIMEOUT)
...
def google_oauth_redirect(request):
    """
    Redirige automáticamente al usuario a Google para autorización
    
    GET /api/auth/google/redirect/
    
    Para navegador: Redirige directamente a Google
    Para API: Usa el header Accept para devolver JSON
    """
    ...
    # Si el cliente pide JSON explícitamente, devolver JSON
    if request.META.get('HTTP_ACCEPT') == 'application/json':
        return Response({
            'auth_url': auth_url
        }, status=status.HTTP_200_OK)
    
    # De lo contrario, redirigir directamente a Google
